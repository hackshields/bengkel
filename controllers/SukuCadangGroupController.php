...
class SukuCadangGroupController extends \yii\web\Controller
{
    ...
    public function actionListData()
    {
        ...
        $query = \app\models\SukuCadangGroup::find()->where(array("jaringan_id" => $user->jaringan_id, "status" => "1"));
    ...
    {
        if ($id == null) {
            $model = new \app\models\SukuCadangGroup();
            if ($model->load($_POST)) {
                if ($model->save()) {
            ...
            $model = \app\models\SukuCadangGroup::find()->where(array("id" => $id))->one();
            if ($model->load($_POST)) {
                if ($model->save()) {
                    return \yii\helpers\Json::encode(array("status" => "OK", "message" => "Simpan Data Berhasil"));
                }
