...
foreach ($files as $file) {
    ...
    ?>
            <tr class="<?php 
    echo "{$file->operation} {$trClass}";
    ?>">
                <td class="file">
                    <?php 
    echo Html::a(Html::encode($file->getRelativePath()), ['preview', 'id' => $id, 'file' => $file->id], ['class' => 'preview-code', 'data-title' => $file->getRelativePath()]);
    ?>
                    <?php 
    if ($file->operation === CodeFile::OP_OVERWRITE) {
        ?>
                        <?php 
        echo Html::a('diff', ['diff', 'id' => $id, 'file' => $file->id], ['class' => 'diff-code label label-warning', 'data-title' => $file->getRelativePath()]);
        ?>
                    <?php 
    }
    ?>
