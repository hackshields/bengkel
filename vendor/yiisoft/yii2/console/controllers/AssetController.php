...
class AssetController extends Controller
{
    ...
    protected function adjustDependency($targets, $bundles)
    {
        ...
        foreach ($map as $bundle => $target) {
            ...
            $targets[$bundle] = Yii::createObject(['class' => strpos($bundle, '\\') !== false ? $bundle : 'yii\\web\\AssetBundle', 'depends' => $depends]);
        }
        return $targets;
    }
    /**
    ...
    protected function saveTargets($targets, $bundleFile)
    {
        ...
        foreach ($targets as $name => $target) {
            if (isset($this->targets[$name])) {
                $array[$name] = ['class' => get_class($target), 'basePath' => $this->targets[$name]['basePath'], 'baseUrl' => $this->targets[$name]['baseUrl'], 'js' => $target->js, 'css' => $target->css];
            } else {
