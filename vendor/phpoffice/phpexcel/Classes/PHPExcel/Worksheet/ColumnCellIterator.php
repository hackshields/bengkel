...
class PHPExcel_Worksheet_ColumnCellIterator extends PHPExcel_Worksheet_CellIterator implements Iterator
{
    ...
    public function next()
    {
        do {
            ++$this->_position;
        } while ($this->_onlyExistingCells && !$this->_subject->cellExistsByColumnAndRow($this->_columnIndex, $this->_position) && $this->_position <= $this->_endRow);
    }
    /**
    ...
    public function prev()
    {
        ...
        do {
            --$this->_position;
        } while ($this->_onlyExistingCells && !$this->_subject->cellExistsByColumnAndRow($this->_columnIndex, $this->_position) && $this->_position >= $this->_startRow);
    }
    /**
    ...
    protected function adjustForExistingOnlyRange()
    {
        if ($this->_onlyExistingCells) {
            while (!$this->_subject->cellExistsByColumnAndRow($this->_columnIndex, $this->_startRow) && $this->_startRow <= $this->_endRow) {
                ++$this->_startRow;
            }
            if ($this->_startRow > $this->_endRow) {
                throw new PHPExcel_Exception('No cells exist within the specified range');
            }
            while (!$this->_subject->cellExistsByColumnAndRow($this->_columnIndex, $this->_endRow) && $this->_endRow >= $this->_startRow) {
                --$this->_endRow;
            }
