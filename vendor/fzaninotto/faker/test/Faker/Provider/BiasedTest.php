...
class BiasedTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testUnbiased()
    {
        ...
        foreach ($this->results as $number => $amount) {
            ...
            $assumed /= 1;
            $this->assertGreaterThan(self::NUMBERS * $assumed * 0.95, $amount, "Value was more than 5 percent under the expected value");
    ...
    public function testLinearHigh()
    {
        ...
        foreach ($this->results as $number => $amount) {
            ...
            // calculate the fraction of the whole area
            $assumed /= pow(1, 2) * 0.5;
            $this->assertGreaterThan(self::NUMBERS * $assumed * 0.9, $amount, "Value was more than 10 percent under the expected value");
