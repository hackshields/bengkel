...
class Generator extends \yii\gii\generators\model\Generator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $params = ['tableName' => $tableName, 'className' => $className, 'queryClassName' => $queryClassName, 'tableSchema' => $tableSchema, 'labels' => $this->generateLabels($tableSchema), 'rules' => $this->generateRules($tableSchema), 'relations' => isset($relations[$tableName]) ? $relations[$tableName] : [], 'ns' => $this->ns, 'enum' => $this->getEnum($tableSchema->columns)];
        ...
        if ($this->useTranslatableBehavior and isset($relations[$langTableName], $relations[$tableName])) {
            ...
            foreach ($relations[$tableName] as $relationName => $relation) {
                ...
                if ($referencedClassName === $langClassName) {
                    ...
                    return ['relations' => $relations, 'translations' => ['fields' => $fields, 'code' => $code]];
                }
            }
        }
