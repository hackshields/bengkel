...
class HTMLPurifier_StringHashParserTest extends UnitTestCase
{
    ...
    {
        $this->assertParse('Simple.txt', array('ID' => 'Namespace.Directive', 'TYPE' => 'string', 'CHAIN-ME' => '2', 'DESCRIPTION' => "Multiline\nstuff\n", 'EMPTY' => '', 'FOR-WHO' => "Single multiline\n"));
    }
    public function testOverrideSingle()
    {
        $this->assertParse('OverrideSingle.txt', array('KEY' => 'New'));
    }
    public function testAppendMultiline()
    {
        $this->assertParse('AppendMultiline.txt', array('KEY' => "Line1\nLine2\n"));
    }
    public function testDefault()
    {
        $this->parser->default = 'NEW-ID';
        $this->assertParse('Default.txt', array('NEW-ID' => 'DefaultValue'));
    }
