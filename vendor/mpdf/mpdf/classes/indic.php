...
class INDIC
{
    ...
    public static function set_syllables(&$o, $s, &$broken_syllables)
    {
        ...
        while ($ptr < strlen($s)) {
            ...
            if (preg_match('/^([CR]m*[N]?(H[ZJ]?|[ZJ]H))*[CR]m*[N]?[A]?(H[ZJ]?|[M]*[N]?[H]?)?[S]?[v]{0,2}/', substr($s, $ptr), $ma)) {
                ...
                if (preg_match('/^(RH|r)?V[N]?([ZJ]?H[CR]m*|J[CR]m*)?([M]*[N]?[H]?)?[S]?[v]{0,2}/', substr($s, $ptr), $ma)) {
                    // From HarfBuzz:
                    ...
                    if (($ptr == 0 || $o[$ptr - 1]['general_category'] < UCDN::UNICODE_GENERAL_CATEGORY_LOWERCASE_LETTER || $o[$ptr - 1]['general_category'] > UCDN::UNICODE_GENERAL_CATEGORY_NON_SPACING_MARK) && preg_match('/^(RH|r)?[sD][N]?([ZJ]?H[CR]m*)?([M]*[N]?[H]?)?[S]?[v]{0,2}/', substr($s, $ptr), $ma)) {
                        // From HarfBuzz:
                        ...
                        if (preg_match('/^(RH|r)?[N]?([ZJ]?H[CR])?([M]*[N]?[H]?)?[S]?[v]{0,2}/', substr($s, $ptr), $ma)) {
                            // From HarfBuzz:
    ...
    public static function set_syllables_khmer(&$o, $s, &$broken_syllables)
    {
        ...
        while ($ptr < strlen($s)) {
            ...
            if (preg_match('/^r?([CR]J?((Z?F)?[N]{0,2})?[ZJ]?G(JN?)?){0,4}[CR]J?((Z?F)?[N]{0,2})?A?((([ZJ]?G(JN?)?)|GZ)|(GJ)?([ZJ]{0,3}MN?(H|JHJR)?){0,4})?(G([CR]J?((Z?F)?[N]{0,2})?|V))?(SZ?)?[v]{0,2}/', substr($s, $ptr), $ma)) {
                ...
                if (preg_match('/^(RH|r)?V((Z?F)?[N]{0,2})?(J|([ZJ]?G(JN?)?[CR]J?((Z?F)?[N]{0,2})?){0,4}((([ZJ]?G(JN?)?)|GZ)|(GJ)?([ZJ]{0,3}MN?(H|JHJR)?){0,4})?(G([CR]J?((Z?F)?[N]{0,2})?|V))?(SZ?)?[v]{0,2})/', substr($s, $ptr), $ma)) {
                    $syllable_length = strlen($ma[0]);
                    $syllable_type = self::VOWEL_SYLLABLE;
                } else {
                    if (preg_match('/^(RH|r)?((Z?F)?[N]{0,2})?(([ZJ]?G(JN?)?)[CR]J?((Z?F)?[N]{0,2})?){0,4}((([ZJ]?G(JN?)?)|GZ)|(GJ)?([ZJ]{0,3}MN?(H|JHJR)?){0,4})(G([CR]J?((Z?F)?[N]{0,2})?|V))?(SZ?)?[v]{0,2}/', substr($s, $ptr), $ma)) {
                        if (strlen($ma[0])) {
                            // May match blank
                            $syllable_length = strlen($ma[0]);
                            $syllable_type = self::BROKEN_CLUSTER;
    ...
    */
    public static $indic_configs = array(
        ...
        9 => array(true, 0x94d, 1, 10, 0),
        10 => array(true, 0x9cd, 1, 9, 0),
        11 => array(true, 0xa4d, 1, 7, 0),
        12 => array(true, 0xacd, 1, 10, 0),
        13 => array(true, 0xb4d, 1, 5, 0),
        14 => array(true, 0xbcd, 1, 12, 0),
        15 => array(true, 0xc4d, 1, 12, 1),
        16 => array(true, 0xccd, 1, 12, 0),
        17 => array(true, 0xd4d, 1, 5, 3),
        18 => array(false, 0xdca, 0, 5, 1),
        /* Sinhala */
