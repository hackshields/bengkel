    ...
    function Gradient($x, $y, $w, $h, $type, $stops = array(), $colorspace = 'RGB', $coords = '', $extend = '', $return = false, $is_mask = false)
    {
        ...
        if ($type == 2) {
            ...
            if ($coords[0] !== false && $coords[1] !== false && $coords[2] !== false && $coords[3] !== false) {
                ...
                if ($angle !== false && $coords[0] !== false && $coords[1] !== false && $coords[2] === false && $coords[3] === false) {
                    ...
                    if ($angle !== false && $coords[0] === false && $coords[1] === false) {
                        ...
                        if ((!isset($angle) || $angle === false) && $coords[0] !== false && $coords[1] !== false) {
            ...
            if ($type == 3) {
                ...
                $angle = isset($coords[5]) ? $coords[5] : false;
                // ?? no effect
        ...
        if (preg_match('/linear-gradient\\((.*)\\)/', $bg, $m)) {
            ...
            if ($startStops == 1) {
                ...
                if (isset($first[1]) && preg_match('/(\\d+)[%]/i', $first[1], $m)) {
                    ...
                    if (!isset($starty) && isset($first[1]) && preg_match('/([0-9.]+(px|em|ex|pc|pt|cm|mm|in))/i', $first[1], $m)) {
                        $tmp = $this->mpdf->ConvertSize($m[1], $this->mpdf->w, $this->mpdf->FontSize, false);
                        if ($tmp) {
                            $starty = $m[1];
                        }
