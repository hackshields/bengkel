...
class PDFBarcode
{
    ...
    public function setBarcode($code, $type, $pr = '')
    {
        ...
        switch (strtoupper($type)) {
            ...
            case 'EAN2':
                ...
                $arrcode['lightmL'] = 7;
                // LEFT light margin =  x X-dim (estimated)
                $arrcode['lightmR'] = 7;
                // RIGHT light margin =  x X-dim (estimated)
                $arrcode['sepM'] = 9;
                // SEPARATION margin =  x X-dim (http://web.archive.org/web/19990501035133/http://www.uc-council.org/d36-d.htm)
                $arrcode['nom-X'] = 0.33;
                // Nominal value for X-dim in mm (http://www.gs1uk.org)
                $arrcode['nom-H'] = 20;
                // Nominal bar height in mm incl. numerals (estimated) not used when combined
                break;
            case 'EAN5':
            ...
            case 'I25B+':
                ...
                // RIGHT light margin =  x X-dim (spec.)
                $arrcode['lightTB'] = 2;
                // TOP/BOTTOM light margin =  x X-dim (non-spec.) - used for bearer bars
                break;
            default:
    ...
    protected function barcode_eanupc($code, $len = 13)
    {
        ...
        if ($upce) {
            ...
            $tmp = substr($code, 4, 3);
            $prod_code = intval(substr($code, 7, 5));
            // product code
            $invalid_upce = false;
