...
class mpdfform
{
    ...
    public function __construct(mPDF $mpdf)
    {
        ...
        // 'xfdf' or 'html'
        $this->formSubmitNoValueFields = true;
        // Whether to include blank fields when submitting data
        $this->formSelectDefaultOption = true;
    ...
    function print_ob_select($objattr, $w, $h, $texto, $rtlalign, $k, $blockdir)
    {
        ...
        if ($this->mpdf->useActiveForms) {
            ...
            if (isset($objattr['size']) && $objattr['size'] < 2) {
                $flags[] = 18;
                //flag 18 = combobox (else a listbox)
    ...
    function SetFormButtonJS($name, $js)
    {
        ...
        if (isset($name) && isset($js)) {
            $this->array_form_button_js[$this->mpdf->_escape($name)] = array('js' => $js);
        }
    }
    function SetFormChoiceJS($name, $js)
    {
        $js = str_replace("\t", ' ', trim($js));
        if (isset($name) && isset($js)) {
            $this->array_form_choice_js[$this->mpdf->_escape($name)] = array('js' => $js);
        }
    }
    ...
    function SetFormText($w, $h, $name, $value = '', $default = '', $title = '', $flags = array(), $align = 'L', $hidden = false, $maxlen = -1, $js = '', $background_col = false, $border_col = false)
    {
        ...
        if ($this->mpdf->onlyCoreFonts) {
            ...
            if (isset($this->mpdf->CurrentFont['subset'])) {
                ...
                $this->mpdf->UTF8StringToArray($default, true);
                // Add characters to font subset
        ...
        if ($this->mpdf->keep_block_together) {
            ...
            if ($this->mpdf->writingHTMLheader || $this->mpdf->writingHTMLfooter) {
                ...
                if ($this->mpdf->ColActive) {
                    $this->mpdf->columnbuffer[] = array('s' => 'ACROFORM', 'col' => $this->mpdf->CurrCol, 'x' => $this->mpdf->x, 'y' => $this->mpdf->y, 'h' => $h);
    ...
    function SetFormChoice($w, $h, $name, $flags, $array, $align = 'L', $js = '')
    {
        ...
        if ($this->mpdf->onlyCoreFonts) {
            ...
            for ($i = 0; $i < count($array['VAL']); $i++) {
                if (isset($this->mpdf->CurrentFont['subset'])) {
                    $this->mpdf->UTF8StringToArray($array['VAL'][$i], true);
                    // Add characters to font subset
                    $this->mpdf->UTF8StringToArray($array['OPT'][$i], true);
                    // Add characters to font subset
                }
                if ($array['VAL'][$i]) {
        ...
        $f = array('n' => $this->formn, 'typ' => 'Ch', 'page' => $this->mpdf->page, 'x' => $this->mpdf->x, 'y' => $this->mpdf->y, 'w' => $w, 'h' => $h, 'T' => $name, 'OPT' => $array, 'FF' => $flags, 'Q' => $align, 'BS_W' => $this->form_border_width, 'BS_S' => $this->form_border_style, 'BC_C' => $this->form_border_color, 'BG_C' => $this->form_background_color, 'style' => array('font' => $this->mpdf->FontFamily, 'fontsize' => $this->mpdf->FontSizePt, 'fontcolor' => $this->mpdf->TextColor));
        if ($js) {
    ...
    function SetJSButton($w, $h, $name, $value, $js, $image_id = 0, $title = '', $flags = array(), $indexed = false, $background_col = false, $border_col = false, $noprint = false)
    {
        ...
        if ($image_id) {
            $this->form_button_icon[$this->mpdf->_escape($name)] = array('pos' => 1, 'image_id' => $image_id, 'Indexed' => $indexed);
        }
        if ($js) {
    ...
    function SetFormButtonText($ca, $rc = '', $ac = '')
    {
        if ($this->mpdf->onlyCoreFonts) {
            ...
            if (isset($this->mpdf->CurrentFont['subset'])) {
                $this->mpdf->UTF8StringToArray($ca, true);
                // Add characters to font subset
            }
    ...
    function _putform_bt($form, $hPt)
    {
        ...
        if ($form['subtype'] == 'radio') {
            ...
            // Radiobutton
            $form['FF'][] = 15;
            // NoToggleOff - must be same as radio button group setting?
            $this->mpdf->_out('/Ff ' . $this->_setflag($form['FF']));
            if ($this->formUseZapD) {
                $this->mpdf->_out('/DA (/F' . $this->mpdf->fonts['czapfdingbats']['i'] . ' 0 Tf ' . $radio_color . ' rg)');
            } else {
