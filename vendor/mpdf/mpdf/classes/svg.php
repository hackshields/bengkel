...
{
    var $svg_font;
    //	array - holds content of SVG fonts defined in image	// mPDF 6
    var $svg_gradient;
    ...
    function svgGradient($gradient_info, $attribs, $element)
    {
        ...
        if ($gradient_info['type'] == 'linear') {
            ...
            if ($spread == 'R' || $spread == 'F') {
                ...
                while ($inside) {
                    ...
                    for ($i = 0; $i < $ns; $i++) {
                        ...
                        $px2 = $px1 + cos($alpha);
                        $py2 = $py1 + sin($alpha);
                        $res1 = _testIntersect($px1, $py1, $px2, $py2, 0, 0, 0, 1);
                        // $x=0 vert axis
                        $res2 = _testIntersect($px1, $py1, $px2, $py2, 1, 0, 1, 1);
                        // $x=1 vert axis
                        $res3 = _testIntersect($px1, $py1, $px2, $py2, 0, 0, 1, 0);
                        // $y=0 horiz axis
                        $res4 = _testIntersect($px1, $py1, $px2, $py2, 0, 1, 1, 1);
                        // $y=1 horiz axis
            ...
            for ($i = 0; $i < $stops; $i++) {
                ...
                $this->mpdf_ref->gradients[$n]['stops'][] = array('col' => $gradient_info['color'][$i]['color'], 'opacity' => $gradient_info['color'][$i]['opacity'], 'offset' => $offset);
        ...
        switch (strtolower($command)) {
            ...
            case 's':
                ...
                for ($i = 0; $i < $ile_argumentow; $i += 4) {
                    ...
                    $this->lastcontrolpoints = array($pdfx - $pdfx2, -($pdfy - $pdfy2));
    ...
    function ConvertSVGSizePixels($size = 5, $maxsize = 'x')
    {
        ...
        $maxsize *= 25.4 / $this->mpdf_ref->dpi;
    ...
    function svgText()
    {
        ...
        if (isset($this->txt_data[2])) {
            ...
            $sw = $this->mpdf_ref->GetStringWidth($txt, true, $OTLdata, $textvar);
    ...
    function svgDefineTxtStyle($critere_style)
    {
        ...
        if (isset($critere_style['font-family']) && $critere_style['font-family'] != 'inherit') {
            ...
            foreach ($aux_fontlist as $f) {
                ...
                if (!$this->mpdf_ref->usingCoreFont && in_array($v, $this->mpdf_ref->available_unifonts) || $this->mpdf_ref->usingCoreFont && in_array($v, array('courier', 'times', 'helvetica', 'arial')) || in_array($v, array('sjis', 'uhc', 'big5', 'gb')) || isset($this->svg_font[$v][$svgfontstyle])) {
                    // mPDF 6
