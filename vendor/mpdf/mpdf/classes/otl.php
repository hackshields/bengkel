...
define("_DICT_NODE_TYPE_SPLIT", 0x1);
define("_DICT_NODE_TYPE_LINEAR", 0x2);
define("_DICT_INTERMEDIATE_MATCH", 0x3);
define("_DICT_FINAL_MATCH", 0x4);
class otl
{
    var $mpdf;
    var $arabLeftJoining;
    ...
    function applyOTL($str, $useOTL)
    {
        ...
        for ($sch = 0; $sch <= $subchunk; $sch++) {
            ...
            if ($useOTL & 0xff && $GSUBscriptTag && $GSUBlangsys && $GSUBFeatures) {
                ...
                if (!isset($this->GSUBdata[$this->GSUBfont])) {
                    if (file_exists(_MPDF_TTFONTDATAPATH . $this->mpdf->CurrentFont['fontkey'] . '.GSUB.' . $GSUBscriptTag . '.' . $GSUBlangsys . '.php')) {
                        ...
                        $this->GSUBdata[$this->GSUBfont] = array('rtlSUB' => array(), 'rphf' => array(), 'rphf' => array(), 'pref' => array(), 'blwf' => array(), 'pstf' => array(), 'finals' => '');
                    }
                }
                    ...
                    for ($i = 0; $i < count($this->OTLdata); $i++) {
                            ...
                            if ($this->OTLdata[$i]['uni'] == 0xfeb3 || $this->OTLdata[$i]['uni'] == 0xfeb4 || $this->OTLdata[$i]['uni'] == 0xfebb || $this->OTLdata[$i]['uni'] == 0xfebc) {
                                ...
                                if ($this->OTLdata[$i]['uni'] == 0xfe94 || $this->OTLdata[$i]['uni'] == 0xfea2 || $this->OTLdata[$i]['uni'] == 0xfeaa) {
                                    ...
                                    if ($this->OTLdata[$i]['uni'] == 0xfe8e || $this->OTLdata[$i]['uni'] == 0xfec2 || $this->OTLdata[$i]['uni'] == 0xfede || $this->OTLdata[$i]['uni'] == 0xfeda || $this->OTLdata[$i]['uni'] == 0xfb93) {
                                        ...
                                        if ($this->OTLdata[$i]['uni'] == 0xfeae || $this->OTLdata[$i]['uni'] == 0xfef2 || $this->OTLdata[$i]['uni'] == 0xfef0 || $this->OTLdata[$i]['uni'] == 0xfef4 || $this->OTLdata[$i]['uni'] == 0xfbe9 || $this->OTLdata[$i]['uni'] == 0xfbfd || $this->OTLdata[$i]['uni'] == 0xfbff) {
                                            $checkpos = $i - 1;
                                            while (isset($this->OTLdata[$checkpos]) && strpos($this->GlyphClassMarks, $this->OTLdata[$checkpos]['hex']) !== false) {
                                                $checkpos--;
                                            }
                        ...
                        if (!isset($this->OTLdata[$i]['GPOSinfo']['kashida'])) {
                            if (strpos($this->GSUBdata[$this->GSUBfont]['finals'], $this->OTLdata[$i]['hex']) !== false) {
                                ...
                                if (strpos('0FEAE 0FEF0 0FEF2', $this->OTLdata[$i]['hex']) !== false) {
            ...
            if ($this->shaper == "K" || $this->shaper == "T" || $this->shaper == "L" || $scriptblock == UCDN::SCRIPT_TIBETAN) {
                ...
                $newinfo[0] = array('general_category' => UCDN::UNICODE_GENERAL_CATEGORY_FORMAT, 'bidi_type' => UCDN::BIDI_CLASS_BN, 'group' => 'S', 'uni' => 0x200b, 'hex' => '0200B');
                ...
                while ($ptr < count($this->OTLdata)) {
                    ...
                    foreach ($this->GSUBLookups[$lu]['Subtables'] as $c => $subtable_offset) {
                        ...
                        if (isset($this->GSLuCoverage[$lu][$c][$currGID])) {
                            ...
                            if (_OTL_OLD_SPEC_COMPAT_1 && $Type == 4 && !$is_old_spec && strpos('0094D 009CD 00A4D 00ACD 00B4D 00BCD 00C4D 00CCD 00D4D', $currGlyph) !== false) {
    ...
    function get_arab_glyphs($char, $type, &$chars, $i, $scriptTag, $usetags)
    {
        ...
        if ($scriptTag == 'syrc' && $char == '00710') {
            ...
            if (isset($chars[$n]) && isset($this->arabLeftJoining[hexdec($chars[$n])])) {
                ...
                if (isset($chars[$i + 1]) && preg_match('/[\\x{0700}-\\x{0745}]/u', code2utf(hexdec($chars[$n]))) && preg_match('/[\\x{0700}-\\x{0745}]/u', code2utf(hexdec($chars[$i + 1]))) && isset($this->arabGlyphs[$char][4])) {
                    ...
                    if (!isset($chars[$i + 1]) || !preg_match('/[\\x{0700}-\\x{0745}]/u', code2utf(hexdec($chars[$i + 1])))) {
        ...
        if (($type > 0 || $type === 0) && isset($this->arabGlyphs[$char][$type])) {
            ...
            if ($type == 3 && isset($this->arabGlyphs[$char][1])) {
                // if <medial> not defined, but <final>, return <final>
                $retk = 1;
            } else {
                if ($type == 2 && isset($this->arabGlyphs[$char][0])) {
        ...
        for ($i = 1; $i < count($Input); $i++) {
            ...
            if ($this->restrictToSyllable && isset($this->OTLdata[$checkpos]['syllable']) && $this->OTLdata[$checkpos]['syllable'] != $current_syllable) {
                ...
                if (!$Input[$i] && isset($this->OTLdata[$checkpos]) && strpos($class0excl, $this->OTLdata[$checkpos]['hex']) === false) {
                    ...
                    if (isset($this->OTLdata[$checkpos]) && strpos($Input[$i], $this->OTLdata[$checkpos]['hex']) !== false) {
                        $matched[] = $checkpos;
                    } else {
                        return false;
                    }
    ...
    function _bidiSort($ta, $str = '', $dir, &$chunkOTLdata, $useGPOS)
    {
        ...
        for ($i = 0; $i < $numchars; ++$i) {
            if ($chunkOTLdata['char_data'][$i]['uni'] == 8235) {
                ...
                if ($chunkOTLdata['char_data'][$i]['uni'] == 8234) {
                    // LRE
                    ...
                    if ($chunkOTLdata['char_data'][$i]['uni'] == 8238) {
                        // RLO
                        ...
                        if ($chunkOTLdata['char_data'][$i]['uni'] == 8237) {
                            // LRO
                            ...
                            if ($chunkOTLdata['char_data'][$i]['uni'] == 8236) {
                                // PDF
                                ...
                                if ($chunkOTLdata['char_data'][$i]['uni'] == 10) {
                                    // NEW LINE
                        ...
                        if ($chunkOTLdata['char_data'][$i]['uni'] == 8238) {
                            ...
                            if ($chunkOTLdata['char_data'][$i]['uni'] == 8237) {
                                ...
                                if ($chunkOTLdata['char_data'][$i]['uni'] == 8236) {
                                    ...
                                    if ($chunkOTLdata['char_data'][$i]['uni'] == 8294 || $chunkOTLdata['char_data'][$i]['uni'] == 8295 || $chunkOTLdata['char_data'][$i]['uni'] == 8296) {
                                        // LRI // RLI // FSI
                                        // X5a. With each RLI:
                                        // X5b. With each LRI:
                                        ...
                                        if ($chunkOTLdata['char_data'][$i]['uni'] == 8294 || $fsi === UCDN::BIDI_CLASS_L) {
                                            ...
                                            if ($chunkOTLdata['char_data'][$i]['uni'] == 8295 || $fsi == UCDN::BIDI_CLASS_R || $fsi == UCDN::BIDI_CLASS_AL) {
                                                // RLI or FSI-R
                                        ...
                                        if ($chunkOTLdata['char_data'][$i]['uni'] == 8297) {
                                            // PDI
