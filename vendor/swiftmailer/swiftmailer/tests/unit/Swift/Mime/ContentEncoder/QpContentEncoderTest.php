...
class Swift_Mime_ContentEncoder_QpContentEncoderTest extends \SwiftMailerTestCase
{
    ...
    public function testPermittedCharactersAreNotEncoded()
    {
        ...
        foreach (array_merge(range(33, 60), range(62, 126)) as $ordinal) {
            ...
            $charStream->shouldReceive('readBytes')->once()->andReturn(array($ordinal));
    ...
    public function testLinearWhiteSpaceAtLineEndingIsEncoded()
    {
        ...
        $charStream->shouldReceive('readBytes')->once()->andReturn(array(0x9));
        $charStream->shouldReceive('readBytes')->once()->andReturn(array(0x9));
        $charStream->shouldReceive('readBytes')->once()->andReturn(array(0xd));
        $charStream->shouldReceive('readBytes')->once()->andReturn(array(0xa));
    ...
    public function testDecimalByte61IsEncoded()
    {
        ...
        */
        ...
        $charStream->shouldReceive('flushContents')->once();
        $charStream->shouldReceive('importByteStream')->once()->with($os);
        $charStream->shouldReceive('readBytes')->once()->andReturn(array(61));
        $charStream->shouldReceive('readBytes')->zeroOrMoreTimes()->andReturn(false);
        $encoder = new Swift_Mime_ContentEncoder_QpContentEncoder($charStream);
        $encoder->encodeByteStream($os, $is);
