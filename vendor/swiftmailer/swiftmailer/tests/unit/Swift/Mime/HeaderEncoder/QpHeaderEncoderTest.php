...
class Swift_Mime_HeaderEncoder_QpHeaderEncoderTest extends \SwiftMailerTestCase
{
    ...
    public function testSpaceAndTabNeverAppear()
    {
        ...
        $charStream->shouldReceive('readBytes')->atLeast()->times(6)->andReturn(array(ord('a')), array(0x20), array(0x9), array(0x20), array(ord('b')), false);
    ...
    public function testEqualsAndQuestionAndUnderscoreAreEncoded()
    {
        /* -- RFC 2047, 4.2.
           ...
           */
        $charStream = $this->_createCharacterStream();
        $charStream->shouldReceive('readBytes')->once()->andReturn(array(ord('=')));
        $charStream->shouldReceive('readBytes')->once()->andReturn(array(ord('?')));
        $charStream->shouldReceive('readBytes')->once()->andReturn(array(ord('_')));
        $charStream->shouldReceive('readBytes')->zeroOrMoreTimes()->andReturn(false);
        $encoder = $this->_createEncoder($charStream);
    ...
    public function testOnlyCharactersAllowedInPhrasesAreUsed()
    {
        /* -- RFC 2047, 5.
                ...
                */
        $allowedBytes = array_merge(range(ord('a'), ord('z')), range(ord('A'), ord('Z')), range(ord('0'), ord('9')), array(ord('!'), ord('*'), ord('+'), ord('-'), ord('/')));
        foreach (range(0x0, 0xff) as $byte) {
            $char = pack('C', $byte);
            $charStream = $this->_createCharacterStream();
            $charStream->shouldReceive('readBytes')->once()->andReturn(array($byte));
            $charStream->shouldReceive('readBytes')->zeroOrMoreTimes()->andReturn(false);
            $encoder = $this->_createEncoder($charStream);
